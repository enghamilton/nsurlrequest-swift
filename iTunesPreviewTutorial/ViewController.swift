...
class ViewController: UIViewController, UITableViewDataSource, UITableViewDelegate  {
    ...
    func callingWebservice(searchTerm: String){
        ...
        if let escapedSearchTerm = itunesSearchTerm.stringByAddingPercentEncodingWithAllowedCharacters(NSCharacterSet.URLQueryAllowedCharacterSet()) {
            ...
            let urlPath = "https://pizzaria2.000webhostapp.com/android_connect/get_all_products.php"
    ...
    {
        do{
            if let jsonResult = NSString(data: data, encoding: NSUTF8StringEncoding) as? String {
                dispatch_async(dispatch_get_main_queue(), {
                    
                    let myAlert = UIAlertView(title: "iOS RESTful", message: "Hello \( jsonResult )", delegate: nil, cancelButtonTitle: "Cancelar")
                    myAlert.show()
                })
            }
        }
